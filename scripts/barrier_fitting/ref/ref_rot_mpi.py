...
def write_orca_input(fname, atoms, charge, mult, nprocs, maxcore_mb, step, angle_deg):
    with open(fname, 'w') as f:
        ...
        f.write(f"\n# Constrained methyl scan — step {step:03d}, angle = {angle_deg:.1f} deg\n\n")
...
def main():
    ...
    parser.add_argument("--ntasks", type=int,  default=15,    help="SLURM ntasks / ORCA nprocs (default 15)")
    parser.add_argument("--mem",    type=str,  default="300g",
                        help="SLURM mem string, e.g. 300g, 128000m (default 300g)")
    ...
    os.makedirs(f"Me{AXIS_ATOM2-1}/scan", exist_ok=True)

    for step, angle_deg in enumerate(angles):
        theta   = np.deg2rad(angle_deg)
        rotated = rotate_atoms(atoms, pivot, axis, theta, ROTOR_H)
        fname   = f"Me{AXIS_ATOM2-1}/scan/step_{step:03d}.inp"
    ...
    print(f"  ntasks/nprocs : {args.ntasks}")
    print(f"  mem (total)   : {args.mem}  ({total_mem_mb} MB)")
    print(f"  %maxcore      : {maxcore_mb} MB per core")
